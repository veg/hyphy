                       ...
                       // VERBOSITY_LEVEL = 10;
						...
						else
						{
                            ...
                            else // MEME
                            {
                                ...
                                global      mixingPleaves     =         0.5; mixingPleaves :< 1-1e-9; mixingPleaves :> 1e-9;
                                ...
                                PopulateModelMatrix              ("MGMatrix1leaves",  positionFrequencies, "alpha", "", "beta1leaves");
                                PopulateModelMatrix              ("MGMatrix2leaves",  positionFrequencies, "alpha", "", "beta2leaves");
                                AC := saveNucs__[0];
                                AT := saveNucs__[1];
                                CG := saveNucs__[2];
                                CT := saveNucs__[3];
                                ...
                                Model 		MG1leaves	=		  ("Exp(MGMatrix1leaves)*mixingPleaves+Exp(MGMatrix2leaves)*(1-mixingPleaves)",codonFrequencies,EXPLICIT_FORM_MATRIX_EXPONENTIAL);
                                Model 		MG1		    =		  ("Exp(MGMatrix1)*mixingP+Exp(MGMatrix2)*(1-mixingP)",codonFrequencies,EXPLICIT_FORM_MATRIX_EXPONENTIAL);
                                Tree	    siteTree = treeString;
                                
                                leaf_names = TipName (siteTree, -1);
                                for (lid = 0; lid < Columns (leaf_names); lid += 1) {
                                    SetParameter (^"siteTree.`leaf_names[lid]`", MODEL, MG1leaves);
                                }
                                ...
                                nsFactor1               :< 1;
                                global      nsFactor2 =  1;
                                global      nsFactor2leaves = 1;
                                global      nsFactor1leaves = 1;
                                nsFactor1leaves:<1;
                                ...
                                doneSites    = {filteredData.unique_sites,13};
                                fullSites    = {filteredData.sites,13};						
                                labels       = {{"beta1","beta2","beta1[leaves]","beta2[leaves]","weight1","weight2","weight1[leaves]","weight2[leaves]","Length_scaler","LRT","p-value","q-value","Full Log(L)"}};
 								...
 								ReplicateConstraint ("this1.?.beta2leaves:=nsFactor2leaves*this2.?.synRate__",siteTree,codonTree);
 								ReplicateConstraint ("this1.?.beta1leaves:=nsFactor1leaves*sFactor*this2.?.synRate__",siteTree,codonTree);
						...
						if (MPI_NODE_COUNT<=1)
						{
							...
							for (siteCount = 0; siteCount < filteredData.sites; siteCount += 1)
							{
								...
								if (alreadyDone[siteMap] == 0)
								{
									...
									{   
                                        ...
                                        else
                                        {
 										...
 										/*if (siteCount == 4) {
                                            Export (siteLF, siteLikelihood);
                                            fprintf (stdout, siteLF);
                                            LIKELIHOOD_FUNCTION_OUTPUT = 7;
                                            fprintf ("/Volumes/sergei-raid/Desktop/meme.fit", CLEAR_FILE, siteLikelihood);
                                            return 0;
                                        }*/
                                ...
                                if (cOptions == 10)
                                {
                                    ReportSiteMEMEInternal (siteCount, siteMap, -1);
                                }
                                else
                                {
						...
						else
						{
                            ...
                            if (cOptions == 10)
                            {
                                bySiteCache      = {filteredData.sites, 6};
                                toDoList         = {};
                                ...
                                "MPI_NEXUS_FILE_RETURN = {};
                                 ...
                                 _vtr = {{\"nsFactor1\", \"nsFactor2\", \"sFactor\", \"mixingP\",\"nsFactor1leaves\", \"nsFactor2leaves\", \"mixingPleaves\"}};
                            ...
                            if (cOptions == 10) {
                                ...
                                qValues = qValues % 0;
                                for (siteCount = 0; siteCount < filteredData.sites; siteCount += 1) {
                                    ReportSiteMEMEInternal (siteCount, dupInfo[siteCount], qValues[siteCount][1]);				 
                                }
