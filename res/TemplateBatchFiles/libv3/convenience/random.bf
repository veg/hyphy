...
random.PI = 4*Arctan (1);

random.LogFactorial.precomp = {{ 0.000000000000000,
             0.000000000000000,
             0.693147180559945,
             1.791759469228055,
             3.178053830347946,
...
lfunction random.LogFactorial(n) {
     if (n > 254) {
         x = n + 1;
         return (x - 0.5) * Log (x) - x + 0.5 * Log (2 * ^"random.PI") + 1.0 / (12.0 * x);
     } else {
         return (^"random.LogFactorial.precomp")[n];
     }
 }

lfunction random.number (from, to) {
...
lfunction random.poisson (lambda) {
 if (lambda < 30) {
     ...
     c = 0.767 - 3.36 / lambda;
     beta = ^"random.PI" / Sqrt (3.0 * lambda);
     alpha = beta * lambda;
...
lfunction random.normal.standard () {
	/* Box-Muller */
	return Sqrt (-2*Log (Random (1e-26,1)))*Cos (2 * ^"random.PI" * Random (1e-26,1));
}

/*-------------------------------------------------*/

lfunction random.binomial (p, N) {
