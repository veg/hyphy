...
function utility.map(object, lambda_name, transform) {
    ...
    if (Type(object) == "AssociativeList") {
        ...
        utility.map.keys = Rows(object); ^ (lambda_name): = object[utility.map.keys[utility.map.k]];
...
function utility.filter(object, lambda_name, condition) {
    ...
    if (Type(object) == "AssociativeList") {
        utility.filter.keys = Rows(object); ^ (lambda_name): = object[utility.filter.keys[utility.filter.k]];
    ...
    if (Type(object) == "Matrix") {
        ...
        utility.filter.columns = Columns(object); ^ (lambda_name): = object[utility.filter.r][utility.filter.c];
...
function utility.forEach(object, lambda_name, transform) {
    ...
    if (Type(object) == "AssociativeList") {
        utility.forEach.keys = Rows(object); ^ (lambda_name): = object[utility.forEach.keys[utility.forEach.k]];
    ...
    if (Type(object) == "Matrix") {
        ...
        utility.forEach.return_object = {
            utility.forEach.rows,
            utility.forEach.columns
...
function utility.populateDict(from, to, value, lambda) {
    ...
    if (Type(lambda) == "String" && Type(value) == "String") {
        Eval("`lambda` = None"); ^ lambda = utility.populateDict.k;
...
function utility.forEachPair(object, key_name, value_name, transform) {
    ...
    if (Type(object) == "AssociativeList") {
        utility.forEachPair.keys = Rows(object); ^ (key_name): = utility.forEachPair.keys[utility.forEachPair.k]; ^ (value_name): = object[utility.forEachPair.keys[utility.forEachPair.k]];
    ...
    if (Type(object) == "Matrix") {
        ...
        utility.forEachPair.return_object = {
            utility.forEachPair.rows,
            utility.forEachPair.columns
        }; ^ (key_name) = {
            {
                utility.forEachPair.r, utility.forEachPair.c
            }
