...
function PopulateModelMatrix (ModelMatrixName&, doRV)
{
	...
	if (doRV)
	{
		incFileName = HYPHY_BASE_DIRECTORY+"TemplateBatchFiles"+DIRECTORY_SEPARATOR+"TemplateModels"+DIRECTORY_SEPARATOR+"defineGamma.mdl";
	...
	if (modelType > 0)
	{
		...
		else
		{
			...
			else
			{
				if (modelType == 3)
				{
					incFileName = HYPHY_BASE_DIRECTORY+"TemplateBatchFiles"+DIRECTORY_SEPARATOR+"TemplateModels"+DIRECTORY_SEPARATOR+"defineHM.mdl";
					ExecuteCommands  ("#include \""+incFileName+"\";");
				}
			}
			m = {{*,AC*c*t,c*t,AT*c*t}
...
function InferTreeTopology(optionFlag)
{
	...
	if (optionFlag == 0)
	{
		incFileName = HYPHY_BASE_DIRECTORY+"TemplateBatchFiles"+DIRECTORY_SEPARATOR+"chooseDistanceFormula.def";
		ExecuteCommands  ("#include \""+incFileName+"\";");
