...
if (relax.model_set == "All") { // run all the models
    ...
    /*
        relax.general_exploratory.fit =  estimators.FitLF (relax.filter_names,
                                        relax.trees,
                                        { "0" : {"DEFAULT" : "relax.ge"}},
        ...
        fprintf ("/tmp/relax.dump.ge", CLEAR_FILE, relax.general_exploratory.fit);
    */

    relax.general_exploratory.fit = io.LoadCacheFromFile ("/tmp/relax.dump.ge");


    selection.io.stopTimer (relax.json [terms.json.timers], "General descriptive model fitting");

    io.ReportProgressMessageMD("RELAX", "ge", "* Log(L) = " + Format(relax.general_exploratory.fit[terms.fit.log_likelihood],8,2));
    io.ReportProgressMessageMD("RELAX", "ge", "* The following baseline rate distribution for branch-site combinations was inferred");
    relax.inferred_ge_distribution = parameters.GetStickBreakingDistribution (models.codon.BS_REL.ExtractMixtureDistributionFromFit (relax.ge.bsrel_model, relax.general_exploratory.fit)) % 0;
    ...
    selection.io.json_store_lf (
                                relax.json,
                                "General exploratory",
                                relax.general_exploratory.fit[terms.fit.log_likelihood],
                                relax.general_exploratory.fit[terms.parameters] + 9 , // +9 comes from CF3x4
                                math.GetIC (relax.general_exploratory.fit[terms.fit.log_likelihood], relax.general_exploratory.fit[terms.parameters] + 9, relax.codon_data_info[terms.data.sample_size]),
                                relax.distribution_for_json
                            );

    selection.io.json_store_branch_attribute(relax.json, "General exploratory", terms.branch_length, 1,
                                                 0,
                                                 selection.io.extract_branch_info((relax.general_exploratory.fit[terms.branch_length])[0], "selection.io.branch.length"));

    relax.k_estimates = selection.io.extract_branch_info((relax.general_exploratory.fit[terms.branch_length])[0], "relax.extract.k");

    io.ReportStatsMD ("*Branch-level intensification parameters", math.GatherDescriptiveStats (utility.Map (utility.Values (relax.k_estimates), "_value_", "0+_value_")));

    selection.io.json_store_branch_attribute(relax.json, "General exploratory k", terms.json.branch_label, 1,
                                                 0,
                                                 relax.k_estimates);

} else {
    relax.general_exploratory = relax.final_partitioned_mg_results;
}

selection.io.stopTimer (relax.json [terms.json.timers], "Overall");
console.log (relax.json);
...
lfunction relax.init.k (lf_id, components, data_filter, tree, model_map, initial_values, model_objects) {
    ...
    parameters.SetConstraint (model.generic.GetGlobalParameter (utility.getGlobalValue("relax.ge.bsrel_model") , terms.AddCategory (utility.getGlobalValue("terms.parameters.omega_ratio"),2)), "1", "global");
