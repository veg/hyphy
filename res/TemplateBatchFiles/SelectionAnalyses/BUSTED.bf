...
io.DisplayAnalysisBanner ({terms.io.info : "BUSTED (branch-site unrestricted statistical test of episodic diversification)
...
busted.terms.optimized_null = "optimized null";
...
codon_data_info[terms.json_] = codon_data_info[terms.file] + ".BUSTED.json";
busted.name_mapping = codon_data_info [utility.getGlobalValue("terms.json.name_mapping")];
...
codon_lists = models.codon.MapCode (codon_data_info[terms.genetic_code]);

_Genetic_Code = codon_data_info[terms.genetic_code];
...
partitions_and_trees = trees.LoadAnnotatedTreeTopology.match_partitions (codon_data_info[utility.getGlobalValue("terms.json.partitions")], busted.name_mapping);
...
busted.model_definitions = busted.io.define_bsrel_models  (busted.terms.FG, busted.terms.BG, codon_frequencies);
...
io.ReportProgressMessageMD ("BUSTED", "GTR", "Log(L) = " + busted.gtr_results[terms.log_likelihood]);

model.ApplyModelToTree ("busted.tree", tree_definition[0], "", {"DEFAULT" : (busted.model_definitions[busted.terms.BG])[terms.model],
                                                             (busted.model_definitions[busted.terms.FG])[terms.model] : Rows (busted.selected_branches)});
...
BUSTED.model_specification[(busted.model_definitions[busted.terms.FG])[terms.model]] = busted.model_definitions;
BUSTED.model_specification[(busted.model_definitions[busted.terms.BG])[terms.model]] = busted.model_definitions;
...
Optimize (busted.MLE_HA, busted.LF);
io.SpoolLF ("busted.LF", codon_data_info[terms.file], None);
...
busted.sample_size             =codon_data_info[terms.sites] * codon_data_info[terms.sequences];
...
busted.json_store_lf                (BUSTED.json, "Unconstrained model",
        ...
        BUSTED.json[busted.terms.background]
        );


busted.siteLogL = {};
(BUSTED.json [terms.json.site_log_likelihood])[busted.terms.unconstrained] = busted.computeSiteLikelihoods ("busted.LF");
...
//    busted.taskTimerStart (1);
    ...
    (BUSTED.json [terms.json.site_log_likelihood])[busted.terms.constrained] = busted.computeSiteLikelihoods ("busted.LF");;
    Optimize (busted.MLE_H0, busted.LF);
    io.SpoolLF ("busted.LF", codon_data_info[terms.file], "null");
    (BUSTED.json [terms.json.site_log_likelihood])[busted.terms.optimized_null] = busted.computeSiteLikelihoods ("busted.LF");;
...
//    busted.taskTimerStop (1);
    ...
    (BUSTED.json [terms.json.evidence_ratios])[busted.terms.constrained] = busted.evidenceRatios ( (BUSTED.json [terms.json.site_log_likelihood ])[busted.terms.unconstrained],  (BUSTED.json [terms.json.site_log_likelihood ])[busted.terms.constrained]);
    (BUSTED.json [terms.json.evidence_ratios])[busted.terms.optimized_null] = busted.evidenceRatios ( (BUSTED.json [terms.json.site_log_likelihood ])[busted.terms.unconstrained],  (BUSTED.json [terms.json.site_log_likelihood ])[busted.terms.optimized_null]);
}
...
*/
(BUSTED.json[terms.json.input])[terms.json.input.filename] = codon_data_info["file"];
(BUSTED.json[terms.json.input])[terms.json.input.sequences] = codon_data_info["sequences"];
(BUSTED.json[terms.json.input])[terms.json.input.sites] = codon_data_info["sites"];
(BUSTED.json[terms.json.input])[terms.json.tree_string] = (tree_definition[0])["string_with_lengths"];
...
fprintf (codon_data_info[terms.json_], CLEAR_FILE, BUSTED.json);
...
function busted.checkForPS (model_parameters) {
   return {terms.omega :Eval (((model_parameters[busted.terms.FG])[terms.omegas])[2]),
           terms.weight : Eval (((model_parameters[busted.terms.FG])[terms.weights])[2])};

}

//------------------------------------------------------------------------------
function busted.constrainTheModel (model_parameters) {
  ExecuteCommands (((model_parameters[busted.terms.FG])[terms.omegas])[2] + ":=1");
}
...
function busted.io.evaluate_branch_lengths (model_parameters, tree_id, fg_set) {
    ...
    for (busted.io.evaluate_branch_lengths.k = 0;
         ...
         if (fg_set [busted.io.evaluate_branch_lengths.b_name]) {
            ...
            busted.io.evaluate_branch_lengths.lexpr = (model_parameters[busted.terms.FG])[terms.length];
         } else {
            //fprintf (stdout, busted.io.evaluate_branch_lengths.b_name, "=>BG\n");
            busted.io.evaluate_branch_lengths.lexpr = (model_parameters[busted.terms.BG])[terms.length];
         }
         Eval (model_parameters[terms.length_parameter] + " = `tree_id`.`busted.io.evaluate_branch_lengths.b_name`." + model_parameters[terms.length_parameter]);
...
function busted.io.define_bsrel_models (foreground_id, background_id, frequencies) {
    ...
    model_parameters =
        {busted.terms.FG: {terms.omegas : {}, terms.weights : {}, terms.f : {}, terms.rate_matrix : {}, terms.length : ""},
         busted.terms.BG: {terms.omegas : {}, terms.weights : {}, terms.f : {}, terms.rate_matrix : {}, terms.length : ""},
         terms.parameters : {terms.global : {}, terms.local : {}}
          };

    for (k = 1; k <= busted.nrates; k +=1) {
        tag = "" + k;
        ((model_parameters[busted.terms.FG])[terms.omegas]) + "`foreground_id`_omega_`tag`";
        ((model_parameters[busted.terms.BG])[terms.omegas]) + "`background_id`_omega_`tag`";
        if (k < busted.nrates) {
            ((model_parameters[busted.terms.FG])[terms.f]) + "`foreground_id`_f_`tag`";
            ((model_parameters[busted.terms.BG])[terms.f]) + "`background_id`_f_`tag`";
        }
    ...
    ((model_parameters[busted.terms.FG])[terms.weights])  = busted._aux.stick_breaking (((model_parameters[busted.terms.FG])[terms.f]));
    ((model_parameters[busted.terms.BG])[terms.weights])  = busted._aux.stick_breaking (((model_parameters[busted.terms.BG])[terms.f]));

    busted.init_values = {"0" : 0.1, "1" : 0.5, "2" : 1};

    ((model_parameters[busted.terms.FG])[terms.omegas])["busted._aux.define_parameter"][""];
    ((model_parameters[busted.terms.BG])[terms.omegas])["busted._aux.define_parameter"][""];

    Eval (((model_parameters[busted.terms.FG])[terms.omegas])[2] + ":<" + terms.range_gte1[terms.upper_bound]);
    Eval (((model_parameters[busted.terms.FG])[terms.omegas])[2] + ":>" + terms.range_gte1[terms.lower_bound]);
    Eval (((model_parameters[busted.terms.BG])[terms.omegas])[2] + ":<" + terms.range_gte1[terms.upper_bound]);

    busted.init_values = {"0" : 0.8, "1" : 0.75};

    ((model_parameters[busted.terms.FG])[terms.f])["busted._aux.define_parameter"][""];
    ((model_parameters[busted.terms.BG])[terms.f])["busted._aux.define_parameter"][""];
    ...
    for (k = 1; k <= busted.nrates; k +=1) {
        ...
        ((model_parameters[busted.terms.FG])[terms.rate_matrix]) + ("Q_`foreground_id`_" + k);
        PopulateModelMatrix			  (((model_parameters[busted.terms.FG])[terms.rate_matrix])[k-1],  busted.nuc, "t",((model_parameters[busted.terms.FG])[terms.omegas])[k-1], "");

        ((model_parameters[busted.terms.BG])[terms.rate_matrix]) + ("Q_`background_id`_" + k);
        PopulateModelMatrix			  (((model_parameters[busted.terms.BG])[terms.rate_matrix])[k-1],  busted.nuc, "t",((model_parameters[busted.terms.BG])[terms.omegas])[k-1], "");
    }


    (model_parameters[busted.terms.BG])[terms.model] = "`background_id`_model";
    (model_parameters[busted.terms.BG])[terms.length] = busted._aux.define_bsrel_model ("`background_id`_model", (model_parameters[busted.terms.BG])[terms.rate_matrix], (model_parameters[busted.terms.BG])[terms.weights], frequencies[terms.codons]);
    (model_parameters[busted.terms.FG])[terms.model] = "`foreground_id`_model";
    (model_parameters[busted.terms.FG])[terms.length] = busted._aux.define_bsrel_model ("`foreground_id`_model", (model_parameters[busted.terms.FG])[terms.rate_matrix], (model_parameters[busted.terms.FG])[terms.weights], frequencies[terms.codons]);
    ...
    ((model_parameters[terms.parameters])[terms.local])[terms.timeParameter ()] = "t";
     ...
     model_parameters[terms.length_parameter] = "t";
...
lfunction busted.io.selectBranchesToTest (tree_definition) {
    ...
    for (k = 0; k < Columns (tree_definition[terms.trees.model_list]); k += 1) {
        model_id = (tree_definition[terms.trees.model_list])[k];
        if (model_id != "") {
...
function busted.json_store_lf (json, name, ll, df, aicc, time, tree_length, tree_string, defs, has_bg) {
    ...
    (((json[terms.json.fits])[name])[terms.json.rate_distributions])[busted.terms.FG] = busted.getRateDistribution (defs, busted.terms.FG);
    if (has_bg) {
        (((json[terms.json.fits])[name])[terms.json.rate_distributions])[busted.terms.BG] = busted.getRateDistribution (defs, busted.terms.BG);
    }
}