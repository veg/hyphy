	...
	if (!tSTA->fadeout)
	{
		if (tSTA->color >= 0)
		{
			...
			for (i = tSTA->startingString; i<tSTA->theText->lLength; i++, finalTick += 11)
			{
				...
				if (buffer[0]&& (thisString->sData[0]=='_'))
				{
					TextFace(1);
					buffer[0]--;
					memcpy (buffer+1,thisString->sData+1,buffer[0]);
	...
	if (itemsInList>=1)
	{
		for (long daItem = 1; daItem <= itemsInList; daItem++)
		{
			...
			while (parentDirID!=fsRtParID)
			{
				...
				if (PBGetCatInfo (&infoRec,false)) return -43;
			...
			if (guiEvent.modifiers&cmdKey)
			{
				if (guiEvent.modifiers&optionKey)
				{
					if ((guiEvent.message&keyCodeMask)==0x1F00)
						...
						if ((guiEvent.message&keyCodeMask)==0x2D00)
		...
		if (mSel>0)
		{
			...
			switch (mSel/0xffff)
			{
				...
				case 131:  // action menu
				{
					...
					if (menuChoice == 1) // cancel execution
					{
						HandleCancel();
					}
					else
					if (menuChoice == 4) // show messages
					{
						ShowMessagesLog();
					}
					else
					if (menuChoice == 6) // display template files
					{
						RunStandardAnalyses();
					}
					else
					if (menuChoice == 8) // rerun last analysus
					{
						OpenBatchFile(false);
						ExecuteBatchFile();
					}
					else
					if (menuChoice == 10) // calculate expression
					{
						if (calculatorMode)
						{
						...
						if (menuChoice == 11) // calculate expression
							ExecuteSelection();
				...
				case 132:  // window menu
				{
					...
					if (menuChoice == 2) // object inspector
					{
						ShowObjectInspector ();
					}		
					else
					if (menuChoice == 3) // cycle thru windows
					{
				...
				case 150: // open submenu
				{
					...
					if (menuChoice==2) // open tree file
					{
						OpenDataFile();
					}
					else
					if (menuChoice==3) // open tree file
					{
						OpenTreeFile();
					}
					else
					if (menuChoice==7) // open text file
					{
						OpenTextFile();
					}
					else
					if (menuChoice==8) // open table
					{
						OpenTable ();
					}
					else
					if (menuChoice==9) // open table
					{
						OpenDatabaseFile (nil);
					}
					else
					if ((menuChoice==11)||(menuChoice==12)) // convert movie file
					{
				...
				case 151: // new submenu
				{
					if (menuChoice==1) // open batch file
						...
						if (menuChoice==2) // open batch file
							NewModel(nil);
						else
							if (menuChoice==3)
								NewChartWindow();
