...
virtual void		Paint (Ptr p) { _HYPlatformLabel::_Paint(p);}
virtual void		Update(Ptr p) { _HYPlatformLabel::_Update(p);}
virtual void		SetDimensions(_HYRect r, _HYRect rel) { _HYComponent::SetDimensions(r,rel);_HYPlatformLabel::_SetDimensions(r,rel);}
virtual	void		SetVisibleSize(_HYRect);
virtual _HYRect		_SuggestDimensions (void);
virtual	void		SetAlignFlags (unsigned char f) {alignFlags = f;}
virtual unsigned char
					GetAlignFlags (void) {return alignFlags;}
		void		SetShadow(bool);
		bool		HasShadow(void) { return visFlags&HY_LABEL_SHADOW; }
	private:
...
virtual bool		GetState   (void) 					{return checkState;}
virtual long		GetSpacing (void) 					{return checkSpacing;}
