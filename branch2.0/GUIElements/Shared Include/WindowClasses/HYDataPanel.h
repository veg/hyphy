...
virtual void		SetFilePath				(_String& s) {filePath=s;}
virtual void		SetSavePath				(_String& s) {savePath=s;}
...
virtual char		WindowKind		(void)  {return HY_WINDOW_KIND_DATAPANEL;}
...
virtual bool		_ProcessOSEvent			(Ptr);
		...
		long		GetLFID					(void) { return lfID; }
		long		GetDSID					(void) { return dataSetID; }
...
virtual bool		IsSaveEnabled	  		(void) {return true;}
virtual bool		IsPrintEnabled	 		(void) {return true;}
virtual void		SetFont					(_HYFont&);
		void		HandleFontChange		(void);

		void		SetLockState	  		(bool r) {cantDeleteLF = r;}
