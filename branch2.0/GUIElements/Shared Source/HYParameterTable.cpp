/*
	...
	Sergei L. Kosakovsky Pond, January 2001-February 2002.
*/
...
{	
	...
	if (e->EventClass()==_hyGlobalLFKillEvent)
	{
		...
		if ((e->EventClass()==_hyGlobalChangeLF)||(e->EventClass()==_hyGlobalChangeLFParams))
		{
			firstArg = e->EventCode().Cut (0,(f=e->EventCode().Find(','))-1);
			k = firstArg.toNum();
			if (k!=GetID())
	...
	if (thisV&&thisV->IsCategory())
	{
		...
		if ((h!=-1)&&(h!=(k-1)))
		{
			if (h<thisCV->GetNumberOfIntervals())
			{
				...
				if (h==thisCV->GetNumberOfIntervals()+1) // produce a plot
				{
		...
		if (rows.lLength==2)
		{
			if (counterT==0)
			{
				...
				if (!(thisV&&thisV2))
					...
					if (!((_CalcNode*)thisV)->MatchSubtree ((_CalcNode*)thisV2))
						thisV2 = nil;
			}
			else
				if (counterT==2)
...
void	_HYParameterTable::HandleCategories(void)
{
	if (lfID>=0)
	{
		...
		_Matrix* 	marginals           = lf->ConstructCategoryMatrix (false, true);
		if (cv->lLength)
		{
...
{	 
	...
	for (k=0; k<rows.lLength; k++)
	{
		...
		if (f>=0)
		{
			...
			if (((_Variable*)RetrieveIthRowVar(rows.lData[k]))->ObjectClass()==TREE)
				treeVar = k+1;
	}
