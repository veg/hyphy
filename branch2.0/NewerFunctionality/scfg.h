...
public:
	ParseNode()		// for root node
		{ itsParent = NULL; itsChar = '\0'; itsIndex = -1; 
		for (int i = 0; i < 95; i++) itsChildren.push_back(NULL); }
	
	ParseNode(ParseNode * p, char c)	
		{ itsParent = p; itsChar = c; itsIndex = -1; }
	
	// default destructor
	~ParseNode()	{ }
	
	// member accessor functions
	void SetParent(ParseNode * p)	{ itsParent = p; }
	ParseNode * GetParent()			{ return itsParent; }
	...
	void SetIndex(int i)		{ itsIndex = i; }
	int GetIndex()				{ return itsIndex; }
	
	void SetChar(char c)		{ itsChar = c; }
	char GetChar()				{ return itsChar; }
	
	int	HowManyChildren()		{ return itsChildren.size(); }
	
	ParseNode * FindChild(char c)	
	{	for (int i = 0; i < itsChildren.size(); i++)
			if (c == itsChildren.at(i)->GetChar()) return itsChildren.at(i); 
		return NULL; }
};
...
virtual 		~Scfg(void); // virtual destructor because of inheritance
	...
	int		GetNumberNTs()						{ return numNT; }
	int		GetNumberTerms()					{ return numT; }
	
	void	SetFirstPT(int fpt)		{ firstPreTerm = fpt; }
	int		GetFirstPreTerm()		{ return firstPreTerm; }
	...
	void	SetProb(int i, int m, double prob) { itsIMProbs[i*numT + m] = prob; }
	...
	double	GetProb(int i, int m)	{ return itsIMProbs[i * numT + m]; }
	...
	String *		AccessString()		{ return itsString; }
	...
	void	GenerateRandomString(bool verbose)	{ GenerateRandomString(verbose, itsString); }
	...
	String *	GetStringFromCorpus(int i)		{ return itsCorpus.at(i); }
	
	void	ClearCorpus()		{ itsCorpus.clear(); }
	
	void	PrintCorpus();
	
	int		GetCorpusSize()		{ return itsCorpus.size(); }
	...
	float	Urn()			{ return pMersenne->Random(); }
	
				/* for non-member access to the random-number generator */
	TRandomMersenne *	AccessMersenne()	{ return pMersenne; }
	...
	void	PrintStringProbs(int s)		{ PrintStringProbs(itsCorpus.at(s)); }
