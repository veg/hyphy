...
GdkColor	menuLine1			= HYColorToGDKColor ((_HYColor){0xA0,0xA0,0xA0}),
			menuLine2			= HYColorToGDKColor ((_HYColor){0x04,0x04,0x04});
		
GdkColor  _BLACKBRUSH_ 			= HYColorToGDKColor ((_HYColor){0,0,0});
...
{	
	...
	if (isPrinting)
 	...
 	GdkColor		whiteC 			= HYColorToGDKColor ((_HYColor){0xff,0xff,0xff}),
 					fillColor 		= HYColorToGDKColor ((_HYColor){102,204,255}),
					...
					else
					{
						if ((cellList->lData[3]==HY_TABLE_COLOR_BOX)||(cellList->lData[3]==HY_TABLE_COLOR_CIRCLE))
						{
							...
							else
							{
								...
								circColor = HYColorToGDKColor((_HYColor){c.R/2,c.G/2,c.B/2});
								...
								circColor = HYColorToGDKColor((_HYColor){c.R/1.25,c.G/1.25,c.B/1.25});
...
bool 		_HYTable::_ProcessOSEvent (Ptr vEvent)
{
	...
	switch (theMessage->theEvent->type)
	{
				...
				if (((k=FindClickedTableCell (lastH-rel.left,lastV-rel.top,h,v))>-1)&&(lastV<rel.bottom-vsShift)&&(lastH<rel.right-hsShift))
				{
					if (theMessage->theEvent->type == GDK_2BUTTON_PRESS)
					{
						if (cursorState == HY_TABLE_SIZE_CURSOR)
						{
							...
							if ((cursorState == HY_TABLE_DRAG_CURSOR || HY_TABLE_PREEDIT_CURSOR == cursorState)&& messageRecipient)
							{
								gdk_pointer_ungrab (((GdkEventButton*)theMessage->theEvent)->time);
		...
		case GDK_MOTION_NOTIFY:
		{
			...
			GdkPoint		downWhere = (GdkPoint){xc-parentWindow->allocation.x,yc-parentWindow->allocation.y};
	
			if (mevent->state & GDK_BUTTON1_MASK) // left button down
			{
				if (cursorState == HY_TABLE_SIZE_CURSOR)
				{
					GdkPoint   currentPoint = (GdkPoint){xc-parentWindow->allocation.x,yc-parentWindow->allocation.y};
					...
					if (cursorState == HY_TABLE_DRAG_CURSOR && activeColumn>=0)
					{
						long h,v,k;
						...
						if (cursorState == HY_TABLE_EDIT_CURSOR || cursorState == HY_TABLE_PREEDIT_CURSOR)
						{
