...
; product_sum.cc:8:     float64x2_t P [5] = {
	movi	v1.2d, 0	; P$2
; product_sum.cc:20:         if (childVector[col_idx] > 0.) {
	ldr	d0, [x1]	; _4, MEM[(hyFloat *)childVector_46(D) + _101 * 8]
; product_sum.cc:20:         if (childVector[col_idx] > 0.) {
	fcmpe	d0, #0.0	; _4
; product_sum.cc:8:     float64x2_t P [5] = {
	mov	v3.16b, v1.16b	; P$3, P$2
	mov	v2.16b, v1.16b	; P$4, P$2
...
; /opt/homebrew/Cellar/gcc/12.2.0/lib/gcc/current/gcc/aarch64-apple-darwin21/12/include/arm_neon.h:15269:   return __builtin_aarch64_ld1v2df ((const __builtin_aarch64_simd_df *) __a);
	...
	ldp	q7, q16, [x0, 32]	; MEM <__Float64x2_t> [(double * {ref-all})tT_84 + 32B], MEM <__Float64x2_t> [(double * {ref-all})tT_84 + 48B], MEM <__Float64x2_t> [(double * {ref-all})tT_84 + 32B]
...
; /opt/homebrew/Cellar/gcc/12.2.0/lib/gcc/current/gcc/aarch64-apple-darwin21/12/include/arm_neon.h:14612:   return __builtin_aarch64_fmav2df (__b, __c, __a);
	fmla	v5.2d, v18.2d, v0.d[0]	; P$0, MEM <__Float64x2_t> [(double * {ref-all})tT_84], _4
	fmla	v4.2d, v17.2d, v0.d[0]	; P$1, MEM <__Float64x2_t> [(double * {ref-all})tT_84 + 16B], _4
	fmla	v3.2d, v16.2d, v0.d[0]	; P$3, MEM <__Float64x2_t> [(double * {ref-all})tT_84 + 48B], _4
	fmla	v1.2d, v7.2d, v0.d[0]	; P$2, MEM <__Float64x2_t> [(double * {ref-all})tT_84 + 32B], _4
	fmla	v2.2d, v6.2d, v0.d[0]	; P$4, MEM <__Float64x2_t> [(double * {ref-all})tT_84 + 64B], _4
L15:
; product_sum.cc:19:     for (long col_idx = 0; col_idx < D; col_idx++, tT+=D) {
...
L4:
; product_sum.cc:20:         if (childVector[col_idx] > 0.) {
	ldr	d0, [x1, x5, lsl 3]	; _4, MEM[(hyFloat *)childVector_46(D) + _101 * 8]
; product_sum.cc:20:         if (childVector[col_idx] > 0.) {
	fcmpe	d0, #0.0	; _4
	bgt	L6		;,
L2:
; product_sum.cc:19:     for (long col_idx = 0; col_idx < D; col_idx++, tT+=D) {
	add	x4, x5, 1	; tmp142, col_idx,
; product_sum.cc:19:     for (long col_idx = 0; col_idx < D; col_idx++, tT+=D) {
	add	x0, x0, 488	; tmp143, transposedMatrix,
; product_sum.cc:20:         if (childVector[col_idx] > 0.) {
	ldr	d0, [x1, x4, lsl 3]	; _4, MEM[(hyFloat *)childVector_46(D) + _101 * 8]
; product_sum.cc:20:         if (childVector[col_idx] > 0.) {
	fcmpe	d0, #0.0	; _4
	bgt	L12		;,
L16:
; product_sum.cc:19:     for (long col_idx = 0; col_idx < D; col_idx++, tT+=D) {
	add	x0, x0, 488	; transposedMatrix, tmp143,
; product_sum.cc:19:     for (long col_idx = 0; col_idx < D; col_idx++, tT+=D) {
	add	x5, x4, 1	; col_idx, tmp142,
	cmp	x4, 60	; tmp142,
	bne	L4		;,
; /opt/homebrew/Cellar/gcc/12.2.0/lib/gcc/current/gcc/aarch64-apple-darwin21/12/include/arm_neon.h:15269:   return __builtin_aarch64_ld1v2df ((const __builtin_aarch64_simd_df *) __a);
...
; /opt/homebrew/Cellar/gcc/12.2.0/lib/gcc/current/gcc/aarch64-apple-darwin21/12/include/arm_neon.h:1003:   return __a * __b;
	...
	fmul	v2.2d, v6.2d, v2.2d	; _47, MEM <__Float64x2_t> [(double * {ref-all})parentConditionals_32(D) + 64B], P$4
	fmul	v4.2d, v16.2d, v1.2d	; _26, MEM <__Float64x2_t> [(double * {ref-all})parentConditionals_32(D) + 32B], P$2
	fmul	v1.2d, v7.2d, v3.2d	; _28, MEM <__Float64x2_t> [(double * {ref-all})parentConditionals_32(D) + 48B], P$3
...
; /opt/homebrew/Cellar/gcc/12.2.0/lib/gcc/current/gcc/aarch64-apple-darwin21/12/include/arm_neon.h:24715:   __builtin_aarch64_st1v2df ((__builtin_aarch64_simd_df *) __a, __b);
	str	q2, [x2, 64]	; _47, MEM <__Float64x2_t> [(double * {ref-all})parentConditionals_32(D) + 64B]
; /opt/homebrew/Cellar/gcc/12.2.0/lib/gcc/current/gcc/aarch64-apple-darwin21/12/include/arm_neon.h:345:   return __a + __b;
	fadd	v3.2d, v4.2d, v1.2d	; tmp139, _26, _28
; /opt/homebrew/Cellar/gcc/12.2.0/lib/gcc/current/gcc/aarch64-apple-darwin21/12/include/arm_neon.h:24715:   __builtin_aarch64_st1v2df ((__builtin_aarch64_simd_df *) __a, __b);
	stp	q4, q1, [x2, 32]	; _26, _28, MEM <__Float64x2_t> [(double * {ref-all})parentConditionals_32(D) + 32B]
; /opt/homebrew/Cellar/gcc/12.2.0/lib/gcc/current/gcc/aarch64-apple-darwin21/12/include/arm_neon.h:345:   return __a + __b;
	fadd	v0.2d, v0.2d, v2.2d	; tmp138, tmp137, _47
	fadd	v0.2d, v0.2d, v3.2d	; <retval>, tmp138, tmp139
; product_sum.cc:48:     if (grandTotal) {
	cbz	x3, L1	; grandTotal,
; /opt/homebrew/Cellar/gcc/12.2.0/lib/gcc/current/gcc/aarch64-apple-darwin21/12/include/arm_neon.h:345:   return __a + __b;
	ldr	q1, [x3]	; *grandTotal_44(D), *grandTotal_44(D)
	fadd	v0.2d, v0.2d, v1.2d	; <retval>, <retval>, *grandTotal_44(D)
; product_sum.cc:49:         *grandTotal = vaddq_f64 (P[2],*grandTotal);
