...
void Parser::reference_like(_Formula& f, _FormulaParsingContext& fpc) {
		...
		if (StartOf(3)) {
			if (la->kind == _MULTIPLY || la->kind == 31 /* "^" */) {
...
void Parser::statement(_ExecutionList &current_code_stream) {
		...
		case _IDENTIFIER: case _FLOAT: case _SINGLE_QUOTE_STRING: case _DOUBLE_QUOTE_STRING: case _NONE_OBJECT: case _OPEN_PARENTHESIS: case _OPEN_BRACE: case _MULTIPLY: case 31 /* "^" */: case 32 /* "&" */: case 36 /* "+" */: case 37 /* "-" */: {
			expression(*f, fpc);
			Expect(_SEMICOLON);
			_parser2013_pushStatementOntoList (this, current_code_stream, f); 
			break;
...
void Parser::hyphy_batch_language() {
		if (_parseExpressionsOnly ()) {
			expression(*f, *fpc);
			while (!(la->kind == _EOF)) {SynErr(56); Get();}
		} else if (StartOf(8)) {
			while (StartOf(7)) {
				block(*hbl_stream);
			}
...
bool Parser::StartOf(int s) {
	...
	static bool set[9][48] = {
		{T,T,T,T, T,T,T,x, x,x,x,x, x,x,T,x, x,T,x,x, x,x,x,x, x,x,x,x, x,x,x,T, T,x,x,x, T,T,x,x, x,x,x,x, x,x,x,x},
		{x,T,T,T, T,T,T,x, x,x,x,x, x,x,T,x, x,T,x,x, x,x,x,x, x,x,x,x, x,x,x,T, T,x,x,x, T,T,x,x, x,x,x,x, x,x,x,x},
		{T,T,T,T, T,T,T,x, x,x,x,x, x,x,T,x, x,T,x,x, x,x,x,x, x,x,x,x, x,x,x,T, T,x,x,x, T,T,x,x, x,x,x,x, x,x,x,x},
		{x,T,T,T, T,T,T,x, x,x,x,x, x,x,T,x, x,T,x,x, x,x,x,x, x,x,x,x, x,x,x,T, x,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x},
		{x,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x, x,T,x,x, x,x,x,x, x,x,x,x, x,x,x,x, x,T,T,T, x,x,x,x, x,x,x,x, x,x,x,x},
		{x,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x, x,x,T,T, T,T,T,T, x,x,x,x},
		{x,x,x,x, x,x,x,x, T,T,T,T, x,x,x,x, x,x,x,x, T,T,T,T, x,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x},
		{x,T,T,T, T,T,T,x, x,x,x,x, x,x,T,x, x,T,x,x, x,x,x,x, T,T,x,T, T,T,T,T, T,x,x,x, T,T,x,x, x,x,x,x, x,x,x,x},
		{T,T,T,T, T,T,T,x, x,x,x,x, x,x,T,x, x,T,x,x, x,x,x,x, T,T,x,T, T,T,T,T, T,x,x,x, T,T,x,x, x,x,x,x, x,x,x,x}
	};
...
void Errors::SynErr(int line, int col, int n) {
	...
	switch (n) {
			...
			case 29: s = coco_string_create(L"CONTINUE expected"); break;
			case 30: s = coco_string_create(L"BREAK expected"); break;
			...
			case 47: s = coco_string_create(L"invalid matrix_row"); break;
			case 48: s = coco_string_create(L"invalid matrix_row"); break;
			case 49: s = coco_string_create(L"this symbol not expected in expression"); break;
			case 50: s = coco_string_create(L"invalid sparse_matrix"); break;
			case 51: s = coco_string_create(L"invalid primitive"); break;
			case 52: s = coco_string_create(L"invalid reference_like"); break;
			case 53: s = coco_string_create(L"this symbol not expected in assignment_op"); break;
			case 54: s = coco_string_create(L"invalid statement"); break;
			case 55: s = coco_string_create(L"invalid block"); break;
			case 56: s = coco_string_create(L"this symbol not expected in hyphy_batch_language"); break;
			case 57: s = coco_string_create(L"invalid hyphy_batch_language"); break;
