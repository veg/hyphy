...
_HYRect		GetScreenDimensions (void)
{
	...
	return  (_HYRect){0,0,gdk_screen_get_height (theScreen),gdk_screen_get_width (theScreen),0};
}
...
_HYColor	GetDialogBackgroundColor (void)
{
	if (hyphyConsoleWindow)
	{
		...
		if (wstyle)
		{
			...
			return (_HYColor){bgc.red/256, bgc.green/256, bgc.blue/256};
		}
	}

	return (_HYColor){255,255,255};
}
...
void hyphy_store_color (GtkWidget *widget, gpointer user_data) {
   ...
   *color_ref = (_HYColor){newColor.red/256,newColor.green/256,newColor.blue/256};
}
