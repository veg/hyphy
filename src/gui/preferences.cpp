				...
				else
				{
					if (prefID->Equal (&windowPositionStr))
					{
						...
						if (prefID->Equal (&recentFilesList))
						{
							...
							if (prefID->Equal (&hyWindowsConsoleFont))
							{
								_List fontList;
...
{	
	...
	recentPaths.InsertElement(&pName,0,true);
		...
		for (long mi=0; mi<recentFiles.lLength; mi++)
		{
			...
			itemFactoryHolder[mi] = (GtkItemFactoryEntry){((_String*)(itemFactoryStrings(mi)))->sData,NULL,hyphy_menu_item_callback,2000+mi,"<Item>"};
