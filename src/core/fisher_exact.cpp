...
int f2xact_(long *, long *, double *, double *, double *, double *, double *,
            double *, double *, long *, long *, long *, long *, long *, long *,
            long *, long *, double *, long *, long *, double *, double *,
            double *, long *, long *, double *);

int f3xact_(long *, long *, long *, long *, double *, long *, double *, long *,
            long *, long *, long *, long *, long *, long *, long *, long *,
            double *, double *, double *);
int f4xact_(long *, long *, long *, long *, double *, double *, long *, long *,
            long *, long *, long *, long *, long *, double *, double *);
int f5xact_(double *, double *, long *, long *, long *, long *, double *,
            long *, long *, long *, long *, long *, long *, long *, bool *);
...
int f10act_(long, long *, long, long *, double *, bool *, double *, long *,
            long *, long *);
...
long *fexact_i4 = nil, *fexact_i5 = nil, *fexact_i7 = nil, *fexact_i10 = nil;

double *fexact_i6 = nil, *fexact_i8 = nil, *fexact_i9 = nil, *fexact_i9a = nil;

long fexact_ldkey = 0, fexact_ldstp = 0;
...
int isort_(long *n, long *ix) {
  long i__, j, m, il[10], kl, it, iu[10], ku, ikey;
...
L30:
  z__ = one / (y * y);
  ret_val = f + (y - half) * log(y) - y + a1 +
            (((-a2 * z__ + a3) * z__ - a4) * z__ + a5) / y;
  return ret_val;
} /* alogam_ */
...
/* ----------------------------------------------------------------------- */
int f10act_(long nrow, long *irow, long ncol, long *icol, double *val,
            bool *xmin, double *fact, long *nd, long *ne, long *m) {
  /* System generated locals */
  long is, ix;
...
/* ----------------------------------------------------------------------- */
int f7xact_(long *nrow, long *imax, long *idif, long *k, long *ks,
            long *iflag) {
  /* System generated locals */
...
/*              (Output) */
/* ----------------------------------------------------------------------- */
int f6xact_(long *nrow, long *irow, long *iflag, long *kyy, long *key,
...
/*              a previous call. */
/* ----------------------------------------------------------------------- */
int f5xact_(double *pastp, double *tol, long *kval, long *key, long *ldkey,
            long *ipoin, double *stp, long *ldstp, long *ifrq, long *npoin,
            long *nr, long *nl, long *ifreq, long *itop, bool *ipsh) {
  /* System generated locals */
  static long itp;

  long i__1, ird, ipn, itmp;
  ...
  if (*ipsh) {
    ...
    HandleApplicationError(
        "Fisher Exact:LDKEY is too small for this problem.  It is not possible "
        "to estimate the value of LDKEY required, but twice the current value "
        "may be sufficient.");
    return 0;
    /*                                  Update KEY */
  L30:
    ...
    if (*itop > *ldstp) {
      HandleApplicationError(
          "Fisher Exact: LDSTP is too small for this problem.  It is not "
          "possible to estimate the value of LDSTP required, but twice the "
...
/*     TOL    - Tolerance.  (Input) */
/* ----------------------------------------------------------------------- */
int f4xact_(long *nrow, long *irow, long *ncol, long *icol, double *dsp,
            double *fact, long *icstk, long *ncstk, long *lstk, long *mstk,
            long *nstk, long *nrstk, long *irstk, double *ystk, double *tol) {
  /* System generated locals */
  long icstk_dim1, icstk_offset, irstk_dim1, irstk_offset, i__1, i__, j, k, l,
      m, n, mn, ic1, ir1, ict, nco, irt, nro, istk;
...
L60:
  ...
  if (irt == ict) {
    --nro;
    --nco;
    f11act_(&irstk[istk * irstk_dim1 + 1], i__, nro,
            &irstk[(istk + 1) * irstk_dim1 + 1]);
    f11act_(&icstk[istk * icstk_dim1 + 1], j, nco,
            &icstk[(istk + 1) * icstk_dim1 + 1]);
  } else if (irt > ict) {
    --nco;
    f11act_(&icstk[istk * icstk_dim1 + 1], j, nco,
            &icstk[(istk + 1) * icstk_dim1 + 1]);
    i__1 = irt - ict;
    f8xact_(&irstk[istk * irstk_dim1 + 1], &i__1, &i__, &nro,
            &irstk[(istk + 1) * irstk_dim1 + 1]);
  } else {
    --nro;
    f11act_(&irstk[istk * irstk_dim1 + 1], i__, nro,
            &irstk[(istk + 1) * irstk_dim1 + 1]);
    i__1 = ict - irt;
    f8xact_(&icstk[istk * icstk_dim1 + 1], &i__1, &j, &nco,
            &icstk[(istk + 1) * icstk_dim1 + 1]);
  }
...
/*     TOL    - Tolerance.  (Input) */
/* ----------------------------------------------------------------------- */
int f3xact_(long *nrow, long *irow, long *ncol, long *icol, double *dlp,
            long *mm, double *fact, long *ico, long *iro, long *it, long *lb,
            long *nr, long *nt, long *nu, long *itc, long *ist, double *stv,
  ...
  if (irow[*nrow] <= irow[1] + *ncol) {
    f10act_(*nrow, &irow[1], *ncol, &icol[1], &val, &xmin, fact, &lb[1], &nu[1],
            &nr[1]);
  }
  if (!xmin) {
...
L90:
  ...
  if (iro[nro] <= iro[irl] + nco) {
    f10act_(nro, &iro[irl], nco, &ico[1], &val, &xmin, fact, &lb[1], &nu[1],
            &nr[1]);
  }
  if (!xmin) {
    if (ico[nco] <= ico[1] + nro) {
      f10act_(nco, &ico[1], nro, &iro[irl], &val, &xmin, fact, &lb[1], &nu[1],
              &nr[1]);
    }
  }
...
L100:
  ...
  nct = ico[1];
  lb[1] =
      (long)((double)((nrt + 1) * (nct + 1)) / (double)(nn + nr1 * nc1s + 1) -
             *tol) -
      1;
  nu[1] =
      (long)((double)((nrt + nc1s) * (nct + nr1)) / (double)(nn + nr1 + nc1s)) -
      lb[1] + 1;
  nr[1] = nrt - lb[1];
  /*                                  Generate a node */
L110:
...
L120:
  ...
  if (lev < nc1s) {
    ...
    nct = ico[lev];
    lb[lev] =
        (long)((double)((nrt + 1) * (nct + 1)) / (double)(nn1 + nr1 * nc1 + 1) -
               *tol);
    nu[lev] =
        (long)((double)((nrt + nc1) * (nct + nr1)) / (double)(nn1 + nr1 + nc1) -
               lb[lev] + 1);
    nr[lev] = nrt - lb[lev];
    goto L120;
  }
  ...
  if (nro == 2) {
    ...
    errMsg = ("Fisher Exact: Stack length exceeded in f3xact.  This problem "
              "should not occur.");
...
/* ----------------------------------------------------------------------- */
int f2xact_(long *nrow, long *ncol, double *table, double *expect,
            double *percnt, double *emin, double *prt, double *pre,
            double *fact, long *ico, long *iro, long *kyy, long *idif,
            long *irn, long *key, long *ldkey, long *ipoin, double *stp,
            long *ldstp, long *ifrq, double *dlp, double *dsp, double *tm,
  ...
  long i__, j, k, n, k1;
  double dd, df;
  long i31, i32, i33, i34, i35, i36, i37, i38, i39, i41, i42, i43, i44, i45,
      i46, i47, i48, ii, kb, kd, ks;
  double pv;
  long i310, i311;
  double ddf;
  long nco, nrb;
  ...
  long nro2, kval = 0, kmax, jkey, last;
  bool ipsh;
  long itmp;
  double dspt;
  ...
  *pre = 0.f;
    ...
    for (j = 1; j <= i__2; ++j) {
      if (table[i__ + j * table_dim1] < -1e-4) {
        HandleApplicationError(errMsg &
                               "All elements of TABLE must be positive.");
        return 0;
      }
  ...
  if (ntot == 0) {
    errMsg = errMsg & "All elements of TABLE are zero.  PRT and PRE are set to "
  ...
  for (i__ = 2; i__ <= i__1; ++i__) {
    ...
    if (iro[i__ - 1] + 1 <= imax / kyy[i__ - 1]) {
      ...
      errMsg = errMsg & "The hash table key cannot be computed because the "
...
L150:
  ...
  if (k1 > 1) {
    ...
    errMsg =
        errMsg &
        "LDKEY is too small.  It is not possible to give thevalue of LDKEY "
        "required, but you could try doubling LDKEY (and possibly LDSTP).";
    ReportWarning(errMsg);
    return 0;
  }
...
L240:
  ...
  if (k1 > 1) {
    ...
    if (dlp[itp] > 0.) {
      ...
      dlp[itp] = 0.;
      f3xact_(&nro2, &irn[nrb], &k1, &ico[kb + 1], &dlp[itp], &ntot, fact,
      ...
      dsp[itp] = dspt;
      f4xact_(&nro2, &irn[nrb], &k1, &ico[kb + 1], &dsp[itp], fact, &iwk[i47],
      ...
      if ((double)(irn[nrb] * ico[kb + 1]) / (double)ntot > emn) {
        ...
        for (i__ = 1; i__ <= i__1; ++i__) {
          i__2 = k1;
          for (j = 1; j <= i__2; ++j) {
            if ((double)(irn[nrb + i__ - 1] * ico[kb + j]) >= ntot * *expect) {
              ++ncell;
            }
            /* L260: */
          }
    ...
    *pre += (double)ifreq * exp(pastp + drn);
    ...
    if (chisq) {
      ...
      *pre += (double)ifreq * exp(pastp + drn) * pv;
      ...
      d__1 = pastp + ddf;
      f5xact_(&d__1, &tol, &kval, &key[jkey], ldkey, &ipoin[jkey], &stp[jstp],
...
/*     feasible. */
/* ----------------------------------------------------------------------- */
int fexact_(long nrow, long ncol, double *table, double expect, double percnt,
            double emin, double *prt, double *pre) {
  long ntot;
  ...
  for (long i = 0; i < ncol * nrow; i++) {
    if (table[i] < 0.) {
      HandleApplicationError(errMsg &
                             "All elements of TABLE must be non-negative.");
      return 0;
    }
  ...
  if (ntot == 0) {
    ReportWarning(errMsg & "All elements of TABLE are zero.  PRT and PRE are "
                           "set to missing values (NaN, not a number).");
    *prt = -1.;
    *pre = -1.;
    return 0;
  }

  long k = nrow + ncol + 1, kk = k * ncol;
  ...
  long *i2 = (long *)MemAllocate(ncol * sizeof(long)),
       ...
       *iiwk = (long *)MemAllocate(Maximum(k * 5 + (kk << 1), ncol * 7 + 800) *
                                   sizeof(long));
  ...
  f2xact_(&nrow, &ncol, table, &expect, &percnt, &emin, prt, pre, i1, i2, i3,
          i3a, i3b, i3c, fexact_i4, &fexact_ldkey, fexact_i5, fexact_i6,
          &fexact_ldstp, fexact_i7, fexact_i8, fexact_i9, fexact_i9a,
          fexact_i10, iiwk, irwk);
