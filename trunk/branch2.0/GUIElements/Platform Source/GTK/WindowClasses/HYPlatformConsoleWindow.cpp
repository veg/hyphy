...
GdkColor		_DARKGREYBRUSH_	   = HYColorToGDKColor((_HYColor){100,100,100});
GdkColor		_WHITEBRUSH_	   = HYColorToGDKColor((_HYColor){255,255,255});
...
void _HYConsoleWindow::_PaintStatusBar(Ptr,bool force)
{
	if (GTK_WIDGET_MAPPED (theWindow))
	{
		...
		if (offBitmap)
		{
			...
			{
				GdkColor blackBrush = HYColorToGDKColor((_HYColor){80,80,80}), 
						orangeBrush = HYColorToGDKColor((_HYColor){255,153,102});
