...
_CString::_CString (void) { 
	allocatedSpace = 0;
	if (FullAlphabet.sLength==0) initFullAlphabet();
...
_Parameter		_CString::FrequencyCompress(unsigned char theAlpha,bool doit)
{
	...
	if ((*theAlphabet).sLength>31) return 1; // can't do much - the alphabet is too large
	char codeLength[256];
	long freqs [256],j,t;
...
_String*	_CString::DecompressFrequency(void)
{
	...
	if (!IsFlag(FREQCOMPRESSION)) return nil; // wrong compression type nothing to do
	unsigned char *codeMaps = new unsigned char [(*theAlphabet).sLength];
	
	if (!codeMaps) warnError( -108); // no memory
	...
	if(i%8) t++; //possible rounding error
	...
	for (k=0,j=t*8;; ) // go by bits
	{
		...
		while (1)
		{
			for (i=8-l%8;i>0; i--)
			{
				if (!(sData[t]&realPowersOf2[i-1])) break;
			}
			if (i) 
			{
				l+=(8-i)-l%8;
...
_Parameter		_CString::LZWCompress (unsigned char theAlpha)
{
	...
	{
		if (!checkTable[sData[p]]) return 1; // symbol not in alphabet - can't compress
...
_String*		_CString::DecompressLZW (void)
{
	...
	if (!sLength||(!IsFlag( LZWCOMPRESSION))) return nil; //nothing to do!
	_List theTable;
...
{
	countDecompress++;
	if (IsFlag(LZWCOMPRESSION)) return DecompressLZW();
	if (IsFlag(FREQCOMPRESSION)) return DecompressFrequency();
	if (compressionType==NOCOMPRESSION) return (_String*)toStr();
	return nil;
}
