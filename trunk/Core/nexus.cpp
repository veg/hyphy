...
bool	SkipUntilNexusBlockEnd (FileState& fState, FILE* file, _String& CurrentLine, long pos)
{
	...
	{	
		...
		if (!CurrentLine.sLength) return false;
		...
		while (newPos<CurrentLine.sLength)
		{
			...
			else
			{
				if (c==';' && ! insideLiteral) // terminate always
				{
					...
					if (stopOnComma && c==',' && ! insideLiteral)
					{
						done = true;
						newPos++;
						break;
...
long	ReadNextNexusEquate (FileState& fState, FILE* f, _String& CurrentLine, long pos2, _String& blank, bool resetP, bool demandSemicolon)
{
	...
	else
	{
		...
		if (res!=2 && demandSemicolon)
		{
			...
			if((res = ReadNextNexusEquate (fState, f, CurrentLine, 0, blank2, resetP, false)))
			{
				blank = blank2;
				return res;
			}
...
void	ProcessNexusTaxa (FileState& fState, long pos, FILE*f, _String& CurrentLine, _DataSet& result)
{
	...
	while (!done)
	{
		if (!FindNextNexusToken (fState, f, CurrentLine, pos)) break;
		...
		if (CurrentLine.startswith (key3))
		{
...
void	ProcessNexusTrees (FileState& fState, long pos, FILE*f, _String& CurrentLine, _DataSet& result)
{
	...
	while (!done)
	{
		...
		if (CurrentLine.beginswith (key2)) // actual tree strings & idents
		{
			_String blank ((unsigned long)10, true);
	...
	if (treeSelected < treeStrings.lLength)
	{
		...
		setParameter (dataFileTreeString, new _FString((*(_String*)treeStrings.lData[treeSelected])),false);
	}
...
bool	ProcessNexusData (FileState& fState, long pos, FILE*f, _String& CurrentLine, _DataSet& result)
{
	...
	while (!done)
	{
		...
		if (CurrentLine.beginswith (key2)) // format instruction
		{
			offSet = key2.sLength;
			while (!done)
			{
				...
				if (blank.beginswith(key21)) // datatype
				{
					...
					else
					{
						...
						if ((blank==_String("DNA"))||(blank==_String("RNA"))||(blank==_String("NUCLEOTIDE")))
						{
							...
							if (blank==_String("PROTEIN") || blank == _String ("BINARY"))
							{
				...
				if (blank.beginswith(key22)) // MISSING
				{
					charSwitcher = 1;
				}
				else
				if (blank.beginswith(key23)) // GAP
				{
					charSwitcher = 2;
				}				
				else
				if (blank.beginswith(key26)) // MATCHCHAR
				{
					charSwitcher = 3;
				}					
				else
				if (blank.beginswith(key27)) // NOLABELS
				{
					labels = false;
				}					
				else
				if (blank.beginswith(key28)) // INTERLEAVE
				{
					fState.interleaved = true;
				}					
				else
				if (blank.beginswith(key24)) // SYMBOLS
				{
				...
				if (blank.beginswith(key25)) // EQUATE
				{
		...
		if (CurrentLine.beginswith (key3)) // matrix instruction
		{
		...
		while ((f = CurrentLine.Find (beginMark,f,-1 ))>=0)
		{
			...
			if (f!=-1) // process 
			{
				...
				if (g!=-1)
				{
					...
					if (blockName.Equal(&assumptions)||blockName.Equal(&sets))
					{
						ProcessNexusAssumptions (fState, g+1, file, CurrentLine, result);
					}
					else
					if (blockName.Equal(&hyphy))
					{
