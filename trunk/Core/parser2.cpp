...
BaseRef _Formula::toStr (_List* matchedNames, bool dropTree) 
{
	...
	_String	* result = new _String((unsigned long)16,true);
	if (!result)
		checkPointer (result);
	...
	while ((fabs((right-left)/MAX(left,right))>machineEps*10.)&&(iterCount<200)) // stuff to do
	{
		...
		{	
			...
			if (t3)
			{
				...
				if (fabs(newCorrection)<machineEps*2.) // correction too small - the root has been found
					return t3;
...
// find a root of the formulaic expression, using Brent's method and a bracketed root.
{
	...
	if (a>b) // autobracket to the left
	{
		...
		if (CheckEqual (a,b)) // autobracket to the right
		{
			dummy.SetValue (a);
			unknown->SetValue(&dummy);
			fa = Compute()->Value();
		...
		if (theObj->IsAVariable())
		{
			if (!includeGlobals)
				if ((((_Variable*)LocateVar(theObj->GetAVariable()))->IsGlobal())||
					(((_Variable*)LocateVar(theObj->GetAVariable()))->ObjectClass()!=NUMBER)) continue;
...
long	   ExecuteFormula (_Formula*f , _Formula* f2, long code)
{
	if (code>=-1)
	{
		...
		if (code>=0)
		{
			LocateVar (code)->SetValue (formulaValue);
			return 1;
		}
		return 0;		
	}
	...
	if (code<-4)
	{
		...
		LocateVar (-code-5)->SetFormula (fFixed);
		return 1;
	}
	if ( code==-3 || code==-4 )
	{
...

{
	...
	for (long i = 0; i<=s.sLength; i++)
	{
																				   ...
																				   // or a parameter list
		{
			...
			if (level<0) 
			{
				if (flagErrors) WarnError (_String("Unbalanced Parentheses in Expression:")&s.Cut(0,i)&"?"&s.Cut(i+1,-1));
				return -2;
			}
			...
			if (levelOps->lLength) // there are some buffered operations left
			{
				...
				{
					if (flagErrors) WarnError (_String ("Syntax error in expression:")&s.Cut(0,i)&"?"&s.Cut(i+1,-1));
					return -2;
				}
			...
			if (lookAtMe ==']')
			{
				...
				{
					if (flagErrors) WarnError (_String ("Unexpected ']': ")&s.Cut(0,i)&"?"&s.Cut(i+1,-1));
					return -2;
				}
				...
				if (mergeMAccess.lLength && mergeMAccess.lData[mergeMAccess.lLength-1] >= 0 && mergeMAccessLevel.lData[mergeMAccessLevel.lLength-1] == level)
				{
					...
					{
						if (flagErrors) WarnError (_String ("Internal error in Parse. Incorrect matrix access token code: ")&s.Cut(0,i)&"?"&s.Cut(i+1,-1));
						return -2;
					}
		...
		if (s.getChar(i) == '=' && s.getChar(i+1) != '=' && (!twoToken || s.getChar(i-1)==':')) // assignment operator
		{
			...
			if (check)
			{
				if (sss)
				{
					...
					if (!f->IsEmpty() || levelData->countitems()!=1 || !((_Operation*)(*levelData)(0))->IsAVariable())
						check = false;
			}
			if (!check)
			{
				if (flagErrors) WarnError (_String("Can't assign like this:")&s.Cut(0,i)&"?"&s.Cut(i+1,-1));
				return -2;
			}
			...
			_Formula  newF;
			if (Parse(&newF,ss,theParent,f2,flagErrors)==-2) 
			{
				inAssignment = false;
				return -2;
			}
		...
		if ( s.getChar(i-1)==':' && (s.getChar(i)=='<' || s.getChar(i)=='>')) // variable bounds
		{
			...
			{
				if (flagErrors) WarnError (_String("Can't set bounds like this:")&s.Cut(0,i)&"?"&s.Cut(i+1,-1));
				return -2;
			}
			...
			{
				if (flagErrors) WarnError (_String("Variable bound must evaluate to a number:")&s.Cut(0,i)&"?"&s.Cut(i+1,-1));
				return -2;
			}
		...
		*/
		...
		if (s.getChar(i) == '[') // opening [
		{
			...
			else
			{
				...
				{
					if (flagErrors) WarnError (_String("[..] must be preceded by an object to index:")&s.Cut(0,i)&"?"&s.Cut(i+1,-1));
					return -2;
				}
		...
		if (s.getChar(i)=='"') // a string literal
		{
			...
			while (i+j<s.sLength)
			{
				...
				if (aChar == '`')
				{
					if (inPlaceID < 0)
						...
						if (j == inPlaceID)
						{
							if (flagErrors) WarnError (_String("Attempted to string substitute an empty quotation:")&s.Cut(0,i+j)&"?"&s.Cut(i+j+1,-1));
							return -2;
						}
						else
						{
			...
			if (inPlaceID >= 0)
			{
				if (flagErrors) WarnError (_String("Unterminated string substitution inside a literal:")&s.Cut(0,i+j)&"?"&s.Cut(i+j+1,-1));
				return -2;
			}
		...
		if (alpha.isAllowed [s.getChar(i)]) // an identifier
		{
			...
			else // a variable
			{
				...
				if (curOpl>2 && curOp[curOpl-1]=='_' && curOp[curOpl-2]=='_') // instant variable refrence
				{
					...
					{
						if (flagErrors) WarnError (_String("Attempted to take value of undeclared variable:")&s.Cut(0,i)&"?"&s.Cut(i+1,-1));
						return -2;
					}
		...
		if ((BinOps.Find (s.getChar(i))!=-1)||(twoToken&&(BinOps.Find(s.getChar(i-1)*(long)256+s.getChar(i))!=-1)))
		{
			...
			if (UnOps.contains(s.getChar(i)))
			{
				...
				else
				{
					...
					if (flagErrors) WarnError ((_String)"Bad unary operator placement " &s.Cut(0,i)&"?"&s.Cut(i+1,-1));
					return -2;
				}
			}
			else
			{
				if (!HalfOps.contains(s.getChar(i)))
				{
					if (flagErrors) WarnError ((_String)"Bad symbols in expression " &s.Cut(0,i)&"?"&s.Cut(i+1,-1));
					return -2;
				}
				else
					twoToken = true;
...
{
	theTree = nil;
	if (Parse (this, s, theParent,nil,errors)==-2)	Clear();
}
