...
{
	if (targetMatrix==sourceMatrix) return;
...
// returns matrix's largest abs value element
{
	if (storageType == 1)
	{
		...
		{
			t = theData[i];
			if ((t<mBench)||(t>bench)) return true;
		}
		return false;
	}
	else
	if (storageType == 0)
	{
		...
		{
			if ((*pData)->IsMaxElement(bench)) return true;
		}
		return false;
	}
	return true;
...
// returns element's position in the buffer (-1 if not found)
{
	...
	// first call, need to set up hashing parameters
	{
		...
		if (!(bufferPerRow = (lDim-overflowBuffer)/hDim)) bufferPerRow = 1;
			...
			if (nn->storageType == 1)
				...
				else
				{
					if (nn->hDim > 0 && nn->vDim == 1) // extract by row
					{
						...
						if (nn->vDim > 0 && nn->hDim == 1) // extract by column
						{
							_SimpleList hL;
...
void		_Matrix::StoreObject (long i, long j, _MathObject* value, bool dup)
{
	...
	if (dup) value = (_MathObject*) value->makeDynamic();
	if (lIndex<0)
	{
		theIndex[-lIndex-2] = i*vDim+j;
...
void	_Matrix::RecursiveIndexSort (long from, long to, _SimpleList* index)
{
	...
	if (middle)
		while ((middle-bottommove>=from)&&(theData[middle-bottommove]>=theData[middle])) bottommove++;
	if (from<to)
		while ((middle+topmove<=to)&&(theData[middle+topmove]<=theData[middle])) topmove++;
...
_PMathObj		_Matrix::pFDR (_PMathObj classes)
{
	...
	else
	{
		if ((GetVDim () != 1 && GetHDim() != 1) || GetVDim()*GetHDim() < 1)
			...
			if (classes->ObjectClass () != NUMBER || classes->Value() > 1. || (pVal = classes->Value()) < 0.0)
...
void		_Matrix::SimplexHelper2 (long& pivotIndex, long columnToExamine, _Parameter eps)
{
	...
	pivotIndex = -1; 
	for (; i<m; i++)
		if (theData[(i+1)*vDim+columnToExamine+1] < -eps) break;
	if (i>=m)
...
void		_Matrix::SimplexHelper3 (long i1, long k1, long ip, long kp)
{
	...
	for (long k=0; k<=k1+1; k++)
		if (k-1 != kp)	theData[(ip+1)*vDim+k] *= -piv;
				  ...
				  // an to avoid goto statements
		{
...

			{
				for (long i=1; i<=m; i++)
				{
					...
					if (t<0.0) m1++; else if (t>0.0) m2++; else m3++;
					if ((*this)(i,0) < 0.0)
					{
			...
			// copy coefficients into the temp matrix, sorting the constraints in the <=, >= and == order
			{
				for (long i=1, t1=0, t2=0, t3=0; i<=m; i++)
				{
					_Parameter t   = (*this)(i,n+1);
					long	   idx;
					if (t<0.0) idx=1+t1++; else if (t>0.0) idx=1+m1+t2++; else idx=1+m1+m2+t3++;
					for (long j=0; j<=n; j++)
			...
			if (m2+m3) // >= and == constraints exist; origin is not a feasible solution
			{
				...
				{
					_Parameter q = 0.;
					for (long k2 = m1+1; k2<=m; k2++) q += tempMatrix(k2,k);
				...
				while (1) // initial artifical construct
				{
					...
					one: tempMatrix.SimplexHelper3 (m,n-1,ip,pivotColumn);
					if (iposv.lData[ip] >= n+m1+m2)
					{
						long k = 0;
						for (k=0; k<nl1; k++)
							if (l1.lData[k] == pivotColumn) break;
						nl1--;
						for (long i2=k; i2<nl1; i2++) l1.lData[i2] = l1.lData[i2+1];
					}
					else
					{
