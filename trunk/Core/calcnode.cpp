...
{	
	...
	if (p&&(p->ObjectClass()==STRING))
	{
		if (p2&&(p2->ObjectClass()==MATRIX))
		{
			...
			_Variable * toptions  = FetchVar(LocateVarByName (treeOutputAVL));
			long	    xtraChars = 0;
			
			if (toptions && toptions->ObjectClass() == ASSOCIATIVE_LIST)
			{
				_Matrix* rgbColor = (_Matrix*)((_AssociativeList *) toptions->GetValue())->GetByKey (treeOutputBackground, MATRIX);
				if (rgbColor)
				{
				...
				_Constant* fontSizeIn = (_Constant*)((_AssociativeList *) toptions->GetValue())->GetByKey (treeOutputFSPlaceH, NUMBER);
				if (fontSizeIn)
					fontSize = fontSizeIn->Value();
				
				fontSizeIn = (_Constant*)((_AssociativeList *) toptions->GetValue())->GetByKey (treeOutputRightMargin, NUMBER);
				if (fontSizeIn)
				{
				...
				if (fontSizeIn = (_Constant*)((_AssociativeList *) toptions->GetValue())->GetByKey (treeOutputXtraMargin, NUMBER))	
			...
			(*res) << _String(lw);
 			(*res) << " setlinewidth\n1 setlinecap\n";
			TreePSRecurse (newRoot, (*res), hScale, vScale, ceil(treeWidth), ceil(treeHeight), fontSize/2, labelWidth-fontSize/2, (toptions && toptions->ObjectClass()==ASSOCIATIVE_LIST)?(_AssociativeList*)toptions->GetValue():nil); 
...
void	_TheTree::TreePSRecurse (node<nodeCoord>* currNode, _String&res, _Parameter hScale, _Parameter vScale, long hSize, long vSize, long halfFontSize, long shift, _AssociativeList* outOptions)
{
	...
	varName = varName.Cut (varName.Find('.')+1,-1);
	...
	long			 vcl = vSize-currNode->in_object.v*vScale,
	...
	if (descendants)
	{
		...
		{ // extra layer to make Borland C++ happy
		for (long k = 1; k<=descendants; k++)
		{
			node<nodeCoord>* child = currNode->go_down(k);
			TreePSRecurse (child, res, hScale, vScale, hSize, vSize,halfFontSize,shift,outOptions);
			if (k==1)
				hc1 = child->in_object.v;
			else
				if (k==descendants)
					hc2 = child->in_object.v;
		}
		}
		char doVLines = 3;
