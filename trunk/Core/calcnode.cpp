...
{	
	...
	if (p&&(p->ObjectClass()==STRING))
	{
		if (p2&&(p2->ObjectClass()==MATRIX))
		{
			...
			if (toptions)
				treeLayout = toptions->GetByKey (treeOutputLayout, NUMBER)->Value();
			...
			if (treeLayout == 1)
			{
				...
				_Parameter			maxRadius = 0.;
				long				tipID = 0;
				newRoot				= RadialBranchMapping (theRoot,nil,scaling?theParam:nil,treeArcAngle/(pi_const*2.*(tipCount-1)),tipID,maxRadius);
			}
			else
			{
				if (scaling)
