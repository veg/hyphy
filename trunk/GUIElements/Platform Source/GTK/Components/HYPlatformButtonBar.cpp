...
GdkColor	buttonBorder1 = HYColorToGDKColor((_HYColor){0,0,0}),
			buttonBorder2 = HYColorToGDKColor((_HYColor){0x3f,0x3f,0x3f});
...
{
	backFill		   = HYColorToGDKColor ((_HYColor){255,255,255});
	pushed			   = -1;
	saveMousePosH	   = -1;
	saveMousePosV	   = -1;
	lastMouseDown	   = -1;
...
void		_HYButtonBar::_DisplayToolTip	   (void)
{
	...
	if ( h>=0 && h<toolTips.lLength)
	{
		...
		if (toolTip->sLength)
		{
			GdkColor	  toolTipColor = HYColorToGDKColor((_HYColor){0xFF,0xCC,0x66});
			...
			toolTipColor = HYColorToGDKColor((_HYColor){0,0,0});
			gdk_gc_set_foreground(bbGC,&toolTipColor);
